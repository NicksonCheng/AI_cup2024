...
def build_mutichoice_template():
    prompt_template = "你是一个准确和可靠的人工智能助手，请根據用户问题以及外部文檔回答答案可能在哪一個文檔。\n" \
                      "以下是外部文档：\n---" \
                        "{}\n" \
                        "用户问题：\n---" \
                        "{}\n"
...
class LLMPredictor(object):
    def __init__(self, llm_model_name_or_path, device="cuda", **kwargs):
        ...
        self.muti_choice_template=build_mutichoice_template()
    ...
    def predict(self, context, query):
        ...
        context = self.muti_choice_template.format(context, query)
